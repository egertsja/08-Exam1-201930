...
def problem4(point1, point2, n, window):
    ...
    dx = (point2.x-point1.x)/(2*n)
    dy = (point2.y-point1.y)/(2*n)
    px = point1.x
    py = point1.y
    for _ in range(n):
        pnt = rg.Point(px+dx,py+dy)
        pnt.fill_color = point2.fill_color
        px = pnt.x
        py = pnt.y
        pnt.attach_to(window)
    for _ in range(n):
        pnt = rg.Point(px + dx, py + dy)
        pnt.fill_color = point1.fill_color
        px = pnt.x
        py = pnt.y
        pnt.attach_to(window)
    window.render()
